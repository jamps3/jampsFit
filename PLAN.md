...
- [ ] **Workout Summary Sync**: The 2026-07 Dancing capture confirms live workout BPM arrives through standard `2A37` and is decoded. The app infers duration, average BPM, BPM range, and estimated active kcal from contiguous HR streams. Find the packet/characteristic that carries workout mode, exact duration, and watch-calculated workout calories; those fields were not present in the attached unknown log.
...
- [x] **Heart Rate Retrieval**: Avoid app-origin `0x6D` writes because they visibly wake or reboot this watch. Scheduled HR is configured with `1F 01` (5m) / `1F 02` (10m), queried with `2F`, and retained by the watch in eight `35` history pages rather than pushed through `2A37`. jampsFit reassembles default-MTU FEE3 fragments, polls the pages automatically, maps their 5-minute local-time slots, and batches deduplicated samples into Room.
